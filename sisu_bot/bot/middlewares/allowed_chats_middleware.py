    ...
    async def __call__(self, handler, event, data):
        ...
        if isinstance(event, Message):
            ...
            if chat_type == "private":
                ...
                # Разрешаем обычные сообщения в личных чатах (Sisu как персонаж)
                logging.info(f"[Allowed] Message in private chat: {text}")
                return await handler(event, data)

            # ГРУППЫ: только whitelisted, но разрешаем обычные сообщения (Sisu как персонаж)
            if chat_type in ("group", "supergroup"):
                if chat_id in list_allowed_chats():
                    logging.info(f"[Allowed] Message in whitelisted group: {chat_id}")
                    return await handler(event, data)
                else:
                    # Сообщения из неразрешенных групп сохраняются через MessageLoggingMiddleware, но бот не отвечает
                    logging.info(f"[Saved but blocked] Message in unapproved group: {chat_id}")
                    return
