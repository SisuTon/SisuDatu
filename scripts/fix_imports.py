...
def find_invalid_imports(project_root: Path) -> dict:
    """Собирает все невалидные импорты по проекту"""
    invalid = defaultdict(list)
    python_files = list(project_root.rglob('*.py'))
    python_files = [f for f in python_files if '.venv' not in str(f) and '.git' not in str(f) and not f.name.endswith('.bak')]
            ...
            if isinstance(node, ast.ImportFrom):
                ...
                for alias in node.names:
                    ...
                    try:
                        ...
                        if not hasattr(imported_mod, name):
                            invalid[module].append(name)
                    except Exception:
                        invalid[module].append(name)
                ...
                for alias in node.names:
                    ...
                    try:
                        ...
                        invalid[module].append('*')
    return invalid

def auto_export_to_init(module: str, name: str) -> bool:
    """Пытается добавить экспорт в __init__.py соответствующего пакета"""
    parts = module.split('.')
    if len(parts) < 2:
        return False
    # Определяем путь к __init__.py
    pkg_path = Path('.').joinpath(*parts)
    if not pkg_path.exists():
        pkg_path = Path('.').joinpath(*parts[:-1])
    ...
    with open(init_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    export_line = f'from .{name} import *\n'
    if export_line not in lines:
        with open(init_path, 'a', encoding='utf-8') as f:
            f.write(export_line)
        REPORT['fixed_exports'].append(f'{module}: {name}')
        return True
    return False

def mass_diagnose_and_fix():
    project_root = Path('.')
    invalids = find_invalid_imports(project_root)
    for module, names in invalids.items():
        for name in names:
            # Пытаемся автодобавить экспорт
            fixed = auto_export_to_init(module, name)
            if not fixed:
                REPORT['unresolved'].append(f'{module}: {name}')
            else:
                print(f'✅ Добавлен экспорт {name} в {module}/__init__.py')
    # Сохраняем отчёт
    with open('fix_imports_report.txt', 'w', encoding='utf-8') as f:
...
def fix_imports_in_file(file_path: Path, dry_run: bool = False) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        for old_import, new_import in IMPORT_MAPPING.items():
            ...
            # Это специальная обработка для случаев типа 'from app.domain.services import points_service'
...
if __name__ == '__main__':
    ...
    if '--diagnose' in sys.argv:
        print('🔍 Диагностика и автопочинка экспортов...')
        mass_diagnose_and_fix()
        print('📄 Отчёт сохранён в fix_imports_report.txt')
    else:
        main() 