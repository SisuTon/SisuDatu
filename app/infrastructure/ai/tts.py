...
class TTSService:
    """DI-совместимый сервис для работы с TTS (Text-to-Speech)"""
    def __init__(self):
        pass

    async def handle_tts_request(self, msg: Message, text: str):
        return await handle_tts_request(msg, text)

    async def send_tts_motivation(self, msg: Message):
        return await send_tts_motivation(msg)

    async def send_tts_fallback_voice(self, msg: Message):
        return await send_tts_fallback_voice(msg)

    def can_use_tts(self, user_id: int) -> bool:
        return can_use_tts(user_id)

    def register_tts_usage(self, user_id: int):
        return register_tts_usage(user_id)

    def get_tts_limit(self, user):
        return get_tts_limit(user)

    def save_motivation_tts(self):
        return save_motivation_tts()
